...
class GetCommentSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {"replies": {"read_only": True}}

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["user"] = GeneralProfileSerializer(
            context={"request": self.context.get("request")}, read_only=True
        )
        ...
        if reply_depth != None and reply_depth <= 2:
            self.fields["replies"] = GetCommentSerializer(
                many=True, read_only=True, context={"reply_depth": reply_depth + 1}
            )

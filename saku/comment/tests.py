...
class CommentTest(TestCase):
    def setUp(self):
        ...
        self.comment1 = Comment.objects.create(
            ...
            content="first depth comment",
            reply_depth=0,
            date="2022-08-24T14:15:22Z",
            reply_on=None,
        )
        self.comment2 = Comment.objects.create(
            user=self.user,
            auction=self.auction,
            content="second depth comment",
            reply_depth=1,
            date="2022-08-24T14:15:22Z",
            reply_on=self.comment1,
        )
        self.comment3 = Comment.objects.create(
            user=self.user,
            auction=self.auction,
            content="third depth comment",
            reply_depth=2,
            date="2022-08-24T14:15:22Z",
            reply_on=self.comment2,
        )
    ...
    def test_reply_comment_depth2(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            ErrorDetail(string="cant reply on this comment", code="invalid"),

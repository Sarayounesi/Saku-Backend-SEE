...
def get_your_colaberation_list(all_user_bids, all_user_auctions):
    colaberation_count = all_user_auctions.values('created_at__date').annotate(count=Count('id')).order_by('created_at__date').union(
                                all_user_bids.values_list('time__date').annotate(count=Count('id')).order_by('time__date'))
                                ### does union works?
...
def get_others_colaberation_list(user):
    colaberation_count = Bid.objects.filter(auction__user=user).values('time__date').annotate(
                                                        count=Count('id')).order_by('time__date')
    ### how to convert to list better ?
    colaberation_list = []
    for colaberation in colaberation_count:
        colaberation_list.append(colaberation['count'])
...
def get_yearly_income_list(user, year):
    income_list = []
    auctions = Auction.objects.filter(user=user, finished_at__year=year, best_bid__isnull=False)
    for auction in auctions:
        income_list.append(auction.best_bid.price)
    return income_list

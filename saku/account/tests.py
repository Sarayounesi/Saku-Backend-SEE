...
class AccountTest(TestCase):
    ...
    def test_register_failure_repeatetive_email(self):
        User.objects.create_user(username='test_user3', password='Ab654321', email="email@eamil.com")
        url = reverse("account:register")
        data =  {"username" : "test_user2", "password" : "Ab654321", "email" : "email@eamil.com"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(ErrorDetail(string='There is another account with this email', code='invalid'),
                      response.data["email"])

    def test_register_failure_repeatetive_username(self):
        User.objects.create_user(username='test_user3', password='Ab654321', email="email@eamil.com")
        url = reverse("account:register")
        data =  {"username" : "test_user3", "password" : "Ab654321", "email" : "email2@eamil.com"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(ErrorDetail(string='A user with that username already exists.', code='unique'),
    ...
    def test_change_password_failure_not_match_password(self):
        ...
        response = self.client.put(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(ErrorDetail(string='Passwords are not the same', code='invalid'),
